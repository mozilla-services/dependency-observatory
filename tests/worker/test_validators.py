...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("#!/usr/bin/env", Exception(r"Invalid NPM package name. Must match '[@a-zA-Z0-9][\\.-_@/a-zA-Z0-9]{0,213}'")),
    ],
    ids=[
        "name",
        "namespaced name",
...
def test_get_npm_package_name_validation_error(package_name: str, expected_validation_error: Optional[Exception]) -> None:
...
def test_get_npm_package_version_validation_error(package_version: str, expected_validation_error: Optional[Exception]) -> None:
    if expected_validation_error is None:
        assert validators.get_npm_package_version_validation_error(package_version) is None
    else:
